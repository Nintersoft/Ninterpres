...
void TfrmSobre::Principal()
{
	try {
		...
		novo = "Adquirindo ";
		throw Exception (L"Você está conectado à internet?\nHouve um erro de conexão com o servidor");
	}
	if (Atualizado()) {
		lblEstado->Text = L"Já está atualizado!";
		novo = "Adquirindo ";
		...
		try {
			...
			Indicador->Visible = false;
			lblEstado->Text = L"Erro";
			throw Exception (L"Você está certo de que está conectado à internet?\nAlgum erro ocorreu enquanto baixavamos as atualizações..");
		}
	}
